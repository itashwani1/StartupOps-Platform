...
const generateContentWithRetry = async (prompt, retries = 3, delay = 2000) => {
  try {
    ...
    if (retries > 0 && error.message.includes("429")) {
      console.log(
        `Gemini Rate Limit hit. Retrying in ${delay / 1000}s... (Retries left: ${retries})`,
      );
...
Format as JSON with this structure:
{
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "JSON parse failed, returning raw text structure:",
        parseError.message,
      );
      ...
      return {
        slides: [
          {
            number: 1,
            title: "AI Presentation Draft",
            content: [text],
            visuals: "",
            notes: "Parsed as raw text due to formatting error",
          },
        ],
      };
    }
...
Format as JSON:
{
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "JSON parse failed for Q&A, returning raw text structure:",
        parseError.message,
      );
      return {
        qa: [{ question: "AI Generated Q&A", answer: text, keyPoints: [] }],
      };
    }
  } catch (error) {
