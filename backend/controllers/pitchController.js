  ...
  try {
    ...
    if (!startup) {
      return res
        ...
        .json({
          message:
            "Startup not found. Please create your startup profile first.",
        });
    }
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to generate pitch script" });
  }
};
...
exports.generatePresentation = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to generate presentation" });
  }
};
...
exports.generateQA = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to generate Q&A preparation" });
  }
};
